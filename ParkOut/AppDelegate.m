...
didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)devToken {
    ...
    viewController.userInfo.device_token = devTokenBytes;
    
    [Communicator updateDeviceToken:viewController.userInfo];
...
-(void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler{
    NSLog(@"didReceiveRemoteNotification: %d",[[userInfo objectForKey:@"id"] intValue]);
...
-(void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions options))completionHandler{
    ...
    if ([[notification.request.content.userInfo objectForKey:@"id"] intValue] == 1){
...
-(void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void(^)(void))completionHandler{
    ...
    if ([[response.notification.request.content.userInfo objectForKey:@"id"] intValue] == 1){
        NSLog(@"Will update intentions");
        [viewController updateIntentions];
    }
}
